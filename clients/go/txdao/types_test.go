...
// outputCoin, policyId) still present on long-lived mainnet records written
// before the field-naming switch. Both must be supported.
...
func assertUTxO(t *testing.T, utxo UTxO, wantPolicy, wantName, wantCoin string) {
	...
	if utxo.Assets[0].Assets[0].OutputCoin != wantCoin {
		t.Errorf("OutputCoin = %q, want %q", utxo.Assets[0].Assets[0].OutputCoin, wantCoin)
	}
}
